...
class Coder:
    ...
    "2. PACKAGE.JSON: You MUST include a start script: `\"scripts\": { \"start\": \"node server.js\" }`.\n"
    "3. **ERROR BRIDGE (MANDATORY)**: In `server.js`, you MUST add a route to capture frontend errors:\n"
    "   ```javascript\n"
    "   app.use(express.json());\n" // REQUIRED for parsing the fetch body
    "   const fs = require('fs');\n"
    "   app.post('/api/log-error', (req, res) => {\n"
    "     const err = req.body.error;\n"
    "     const msg = `[FRONTEND FATAL] ${err}\\n`;\n"
    "     console.error(msg); // Prints to Terminal for RunManager\n"
    "     fs.appendFileSync('server_errors.txt', msg); // Saves for AI to read\n"
    "     res.json({ success: true });\n"
    "   });\n"
    "   ```\n"
    "4. STATIC SERVING: Import `path`. `app.use('/static', express.static('static'));` and serve `index.html` at `/`.\n\n"

    "FRONTEND RULES (MODERN REACT via CDN):\n"
    "1. **CRITICAL: INDEX.HTML FIRST**: If creating the frontend, start with `index.html`.\n"
    "2. **THE SPY SCRIPT (MANDATORY)**: In `index.html`, inside the `<head>`, add this script FIRST (before any CDNs):\n"
    "   ```html\n"
    "   <script>\n"
    "     window.onerror = function(msg, url, line) {\n"
    "       const errorMsg = `Error: ${msg} at ${url}:${line}`;\n"
    "       console.error('Sending error to backend:', errorMsg);\n"
    "       fetch('/api/log-error', {\n"
    "         method: 'POST',\n"
    ...
    "   </script>\n"
    "   ```\n"
    "3. NO BUILD STEP: Do NOT use Vite/Webpack.\n"
    "4. ARCHITECTURE:\n"
